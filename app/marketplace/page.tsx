...
export default function MarketplacePage() {
  ...
  const filtered = useMemo(() => {
    ...
    // Category
    if (filters.category) {
      result = result.filter((l) => l.inft.skillCategory === filters.category);
    }

    // Tier
    if (filters.tier) {
      result = result.filter((l) => l.inft.tier === filters.tier);
    }

    // Score range
    if (filters.minScore) {
      result = result.filter((l) => l.inft.score >= parseInt(filters.minScore));
    }
    if (filters.maxScore) {
      result = result.filter((l) => l.inft.score <= parseInt(filters.maxScore));
    }

    // Price range
    if (filters.minPrice) {
      result = result.filter((l) => parseFloat(l.priceEther) >= parseFloat(filters.minPrice));
    }
    if (filters.maxPrice) {
      result = result.filter((l) => parseFloat(l.priceEther) <= parseFloat(filters.maxPrice));
    }
    ...
    switch (filters.sortBy) {
      case 'newest':
        ...
        break;
      case 'oldest':
        result.sort((a, b) => a.listedAt - b.listedAt);
        break;
      case 'score_high':
        result.sort((a, b) => b.inft.score - a.inft.score);
        break;
      case 'price_low':
        result.sort((a, b) => parseFloat(a.priceEther) - parseFloat(b.priceEther));
        break;
      case 'price_high':
        result.sort((a, b) => parseFloat(b.priceEther) - parseFloat(a.priceEther));
        break;
      case 'most_viewed':
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            <div className="font-mono text-gray-500 text-lg mb-2">No INFTs found</div>
            <div className="font-mono text-gray-700 text-sm mb-6">
              Try adjusting your filters or mint the first INFT!
            </div>
            <Link
              href="/mint"
